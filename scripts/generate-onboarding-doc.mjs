...
function coverPage() {
  return [
    ...
    new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 80 },
      children: [new TextRun({ text: 'Version 3.0', color: GRAY, size: 22 })],
    }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 240 },
...
function section1() {
  return [
    ...
    bullet('Mileage reimbursement rates are managed in the application by an Admin'),
    bullet('New user accounts require activation by an Admin before access is granted'),
    bullet('Soft-deleted reports are retained in a log accessible to Admins'),
    bullet('A dedicated Analytics section provides organization-wide mileage, reimbursement, property, and approval-speed reporting'),
    spacer(),
...
function section6() {
  return [
    ...
    bullet('Admins may soft-delete a report (it moves to the Deleted Reports log accessible to all Admins).'),
    spacer(),
    h2('6.4  Admin Dashboard'),
    ...
    spacer(),
    h2('6.5  Analytics'),
    p('The Analytics section in the sidebar provides five detailed reporting views for Admin users:'),
    makeTable(
      ['Page', 'Description'],
      [
        ['Overview',         'Radar view — live pipeline status (pending/draft/needs revision), year summary, top employees, top destinations, recent monthly activity, and approval speed highlights.'],
        ['Employee Miles',   'Sortable table of all employees with trip count, total miles, total reimbursed, and average miles per trip. Filter by year and manager.'],
        ['Properties',       'Trip activity by property — trips as origin, trips as destination, total trips, miles, and reimbursement. Filter by year.'],
        ['Monthly Trends',   'Chronological month-by-month breakdown of trips, miles, and reimbursement with inline bar visualization. Filter by year.'],
        ['Approval Metrics', 'Manager turnaround time — average days from submission to decision, reports reviewed, approved, and returned. Color-coded bars (green = fast). Filter by year and manager.'],
      ],
      [22, 78]
    ),
    spacer(),
    h2('6.6  Bulk Excel Export'),
  ...
  return [
    ...
    makeTable(
      ...
      [
        ...
        ['Submitted',     'Report has been sent to all assigned approvers. Trips are locked.',                                    'Any assigned approver or Admin', 'Approve → Approved\nReturn → Needs Revision'],
        ['Needs Revision','Approver returned the report. Employee can edit trips and resubmit.',                                  'Employee (owner)',           'Resubmit → Submitted'],
        ['Approved',      'Report fully approved. Locked permanently. Accounting Excel emailed automatically.',                   'Admin (soft delete only)',   'Soft Delete → (log)'],
      ],
      [18, 37, 25, 20]
    ),
    spacer(),
    noteBox('Note', 'Reports are never hard-deleted through normal workflows. Even soft-deleted reports remain accessible to Admins in the Deleted Reports log.', NAVY, LIGHT),
    br(),
  ]
}
...
function section11() {
  return [
    ...
    spacer(),
    noteBox('Mileage Rate', 'The mileage reimbursement rate is NOT configured via an environment variable. It is managed entirely within the application under Administration → Mileage Rates. This allows the rate to be updated without a code deployment.', GREEN, 'F0FDF4'),
    spacer(),
...
function section13() {
  return [
    ...
    spacer(),
    h2('"I cannot see the Administration or Analytics sections in the sidebar."'),
    p('The Administration and Analytics sections are visible only to Admins. If you should have Admin access, contact Michael Pisano (mpisano@riverwestpartners.com) to have your role updated.'),
    spacer(),
